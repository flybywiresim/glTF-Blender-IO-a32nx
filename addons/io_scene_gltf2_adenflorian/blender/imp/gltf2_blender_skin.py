...
class BlenderSkin():
    ...
    @staticmethod
    def create_armature(gltf, skeletonid, pyskeletonnode, parent):
        """Armature creation."""

        if pyskeletonnode.name is not None:
            name = "Armature_" + pyskeletonnode.name
        else:
            name = "Armature_" + str(skeletonid)
        ...
        pyskeletonnode.blender_armature_name = obj.name
        print('create_armature pyskeletonnode.name: ' + pyskeletonnode.name)
        if parent is not None:
            print('parent node name: ' + gltf.data.nodes[parent].name)
            obj.parent = bpy.data.objects[gltf.data.nodes[parent].blender_armature_name]
    ...
    def set_bone_transforms(gltf, skin_id, bone, node_id, parent):
        ...
        #     parentskin = gltf.data.skins[gltf.data.nodes[parent].skin_id]
        #     if pyskin.skeleton == parentskin.skeleton:
    ...
    def create_bone(gltf, skin_id, node_id, parent):
        ...
        pynode.blender_armature_name = pyskeletonnode.blender_armature_name
    ...
    def create_armature_modifiers(gltf, skin_id):
        ...
        for node_id in pyskin.node_ids:
            ...
            armature_obj = bpy.data.objects[pyskeletonnode.blender_armature_name]
            ...
            bpy.data.objects[pyskeletonnode.blender_armature_name].select_set(True)
            # Makes sure the armature obj was selected last so that it becomes the parent
            bpy.context.view_layer.objects.active = bpy.data.objects[pyskeletonnode.blender_armature_name]
            bpy.context.view_layer.update()
            # Actually does the parenting operation
            bpy.ops.object.parent_set(type='OBJECT', keep_transform=True)

            # obj.parent = bpy.data.objects[pyskeletonnode.blender_armature_name]
            # arma = obj.modifiers.new(name="Armature", type="ARMATURE")
            # arma.object = bpy.data.objects[pyskeletonnode.blender_armature_name]
